    ...
    <div className="space-y-6">
      ...
      <div className="border rounded-md bg-card">
        <Table>
            ...
            <TableBody>
                {consultations?.length === 0 && <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground">No consultations found.</TableCell></TableRow>}
                {consultations?.map((consultation) => (
                    <TableRow key={consultation.id}>
                        ...
                        <TableCell>
                            {consultation.note ? <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">Note saved</Badge> : <span className="text-muted-foreground text-xs">No note</span>}
                        </TableCell>
                        <TableCell className="text-right">
                            <Link href={`/dashboard/consultation/${consultation.id}`}>
                                <Button variant="ghost" size="sm"><Eye className="mr-2 h-4 w-4"/> Open</Button>
                            </Link>
                        </TableCell>
                    </TableRow>
                ))}
